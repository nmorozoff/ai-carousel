...
function buildGrsaiPrompt(
  ...
  const styleShort: Record<string, string> = {
    ...
    "Инфографика с экспертом — тёмная": `Dark premium infographic, not mystical. Same dark navy office (#1A1A2E) and one wooden desk on ALL slides. Woman expert from reference sometimes stands by the desk, sometimes sits at the same desk, sometimes leans slightly toward the viewer, always in this one room with soft shadows.${charHint}${accent} Left side reserved for big Russian headline and short bullet list, right side for the woman and simple charts, same layout on every slide. No English words, no decorative quotes or random icons. ${textBlock}. Photorealistic, 4:5 vertical.`,
    "Тёмный": `Dark premium coaching portrait, not mystical and not funeral. Same deep plum wall and wooden table scene on ALL slides. Woman from reference sometimes sits at the table, sometimes stands next to it, sometimes leans slightly forward, but always in this one room, calm, confident, hopeful.${charHint} Large Russian headline and short text block on the left, clean and readable, same position on every slide. No English words, no decorative elements. ${textBlock}. Photorealistic, 4:5 vertical.`,
